...
exports._controlTransferIn =
  dev => reqType => recipient => request => val => index => len => () =>
  dev.controlTransferIn({ requestType: reqType, recipient: recipient, request: request, value: val, index: index }, len).then(result => Array.from(new Uint8Array(result.data.buffer)));
